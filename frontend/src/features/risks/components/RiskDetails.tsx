  ...
  const handleAddMitigation = async (e: React.FormEvent) => {
    ...
    try {
      await api.post(`/risks/${risk.id}/mitigations`, {
        ...
        assignee: 'Current User', // À connecter à l'auth plus tard
      });
      toast.success("Plan d'action ajouté");
      setNewMitigationTitle('');
      await fetchRisks(); // Refresh pour voir la nouvelle mitigation
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-2 flex-wrap">
        <span className={`px-3 py-1 rounded-full text-xs font-bold border ${
             risk.score >= 15 ? 'bg-red-500/10 text-red-500 border-red-500/20' : 
        ...
        {activeTab === 'overview' ? (
            <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-zinc-900/50 border border-white/5">
                    <h4 className="text-xs text-zinc-500 uppercase font-bold mb-2">Impact</h4>
                ...
                <div className="col-span-2 p-4 rounded-xl bg-zinc-900/50 border border-white/5">
                    ...
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 rounded-full bg-indigo-500 flex items-center justify-center text-[10px]">JD</div>
